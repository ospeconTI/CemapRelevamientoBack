...
import { dmdBusquedaOdataComponent } from "./dmdBusquedaOdata";
...
export class pruebaOdataScreen extends connect(store, TABLE, TABLE_ERROR, MEDIA_CHANGE, SCREEN)(LitElement) {
	constructor() {
		super();
		this.hidden = true;
		this.area = "body";
		...
		this.estilosDmdBusquedaOdata = css`
			.dmd-busqueda-odata {
				width: 80%;
				height: 60%;
			}
			...
			.dmd-busqueda-odata-datos-registro[campo="Documento"] {
				text-align: right;
			}
			.dmd-busqueda-odata-datos-registro[campo="Fecha"] {
				text-align: center;
			}
		`;
	}
	...
	render() {
		return html`
			...
			<dmd-busqueda-odata-component hidden .dataSource=${this.dataSource} .estilos=${this.estilosDmdBusquedaOdata} .config=${this.config} id="cBusqueda"> </dmd-busqueda-odata-component>
		`;
	}
	mostrar(e) {
	...
	stateChanged(state, name) {
		if (name == SCREEN || name == MEDIA_CHANGE) {
			...
			const SeMuestraEnUnasDeEstasPantallas = "-pruebaOdata-".indexOf("-" + state.screen.name + "-") != -1;
...
window.customElements.define("prueba-odata-screen", pruebaOdataScreen);
