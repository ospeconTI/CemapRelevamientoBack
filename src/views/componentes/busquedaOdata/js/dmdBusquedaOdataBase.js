...
export const dmdBusquedaOdataBase = (baseElement) =>
	class extends baseElement {
		...
		firstUpdated() {
			super.firstUpdated();
			let cerrar = this.shadowRoot.querySelector(".dmd-busqueda-odata-cabecera-cerrar");
			let buscar = this.shadowRoot.querySelector(".dmd-busqueda-odata-input-lupa");
			let resetFiltro = this.shadowRoot.querySelector(".dmd-busqueda-odata-input-searchoff");
			if (cerrar) {
			...
			[].forEach.call(this.shadowRoot.querySelectorAll(".dmd-busqueda-odata-datos-titulo"), (element) => {
				if (element.campo) {
		...
		_estadoInicial() {
			this.shadowRoot.querySelector(".dmd-busqueda-odata-input-input").value = "";
			this._filtrarDmdBusqueda(null);
		}
		...
		_filtrarDmdBusqueda(e) {
			...
			if (accion == "ordenar") {
				...
				let input = this.shadowRoot.querySelector(".dmd-busqueda-odata-input-input");
				this.grid = this._filtrarArray(this.dataSource, campoActual, input.value);
			} else if (accion == "cargaInicial" || accion == "resetFiltro") {
				this.shadowRoot.querySelector(".dmd-busqueda-odata-input-input").value = "";
				let campoCargainicial = this.shadowRoot.querySelectorAll(".dmd-busqueda-odata-datos-titulo")[0];
				this._desmarcarSeleccionDmdBusqueda();
				this._tituloRefreshIconoDmdBusqueda();
		...
		_accion(e) {
			...
			let obj = e ? e.currentTarget : null;
			if (obj) {
				if (obj.className == "dmd-busqueda-odata-datos-titulo") {
					accion = "ordenar";
				} else if (obj.className == "dmd-busqueda-odata-input-lupa") {
					accion = "buscar";
				} else if (obj.className == "dmd-busqueda-odata-input-searchoff") {
					accion = "resetFiltro";
				}
			} else {
				accion = "cargaInicial";
		...
		_campoActual() {
			var campoActual = "";
			let titulo = this.shadowRoot.querySelectorAll(".dmd-busqueda-odata-datos-titulo");
