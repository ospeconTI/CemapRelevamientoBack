...
import { getTable as getTable } from "../../../../redux/depositos/actions";
				...
				[].forEach.call(this.shadowRoot.querySelectorAll("[dmd-table-orden]"), (element) => {
					...
					if (element.getAttribute("tipo") == "numerico" && /^\-*\d*\,?\.?\d*$/.test(input.value) && !isValidDate(input.value)) {
						...
						if (dateParts.length == 3) {
							filtro = element.getAttribute("campo") + " eq " + dateParts[2] + "-" + ("0" + dateParts[1]).substring(("0" + dateParts[1]).length - 2) + "-" + ("0" + dateParts[0]).substring(("0" + dateParts[0]).length - 2);
						}
					}
					if (filtro.length > 0) {
						if (oData == "") {
