...
import { set as setCemaps } from "../cemaps/actions";
...
export const login =
	...
	(action) => {
		...
		if (action.type === LOGIN) {
			...
			dispatch(RESTAdd(loginFetch, { username: action.email, password: action.password }, success, action.loginError, "", ""));
	...
	(action) => {
		...
		if (action.type === LOGIN_SUCCESS || action.type === LOGIN_SUCCESS_AUTO) {
			if (!action.payload.receive.Profiles || action.payload.receive.Profiles.length == 0 || action.payload.receive.message || action.payload.receive.length == 0) {
				...
				dispatch(setCemaps(action.payload.receive.Profiles[0].Perfil.Cemaps));
				dispatch(goTo("inicial"));
				//viewMode("main");
			}
		}
	};
